...
One major difference between M2 and M1 is that the accelerometer has a maximum absolute value of 3.0g. Any values below -3.0g are clipped to -3.0g. Any values higher than +3g are clipped to 3.0g. In addition, the speed of transmission during the message on milestone 2 can vary by 20% from its initial value. The timing and all other parameters for unit "dot" timing and the travel times for the key are the same as in milestone 1.

A second major difference is that while the transmission speed for milestone 1 was fixed throughout a single run, the operator in milestone 2 may vary their transmission speed randomly by up to 20% during the transmission of a single real-world message. Your decoding algorithm must be dynamic enough to track reasonable drifts in dot and dash lengths.
