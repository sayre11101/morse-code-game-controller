...
One major difference between M2 and M1 is that the accelerometer has a maximum absolute value of 3.0g. Any values below -3.0g are clipped to -3.0g. Any values higher than +3.0g are clipped to 3.0g. The timing and all other parameters for unit "dot" timing and the travel times for the key are the same as in milestone 1.
